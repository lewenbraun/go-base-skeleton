# go-base-skeleton — agent instructions
...
- **Before commit**: golang-lint (config is project/.golangci.yml) +
...
- After finishing, re-scan the latest user message and verify nothing was
  skipped
- Run `make -C project check` after any code change; report linter output
  verbatim

## Project essentials

- Skeleton root holds agent infrastructure only; the Go project lives in
  `project/` (module `github.com/lewenbraun/go-base-skeleton/project`)
- Forking the skeleton for a product: rename `project/`, the module path,
  `BIN` in the Makefile and the `cmd/<name>/` directory
- Tooling pinned in project/go.mod `tool` block: golangci-lint,
  govulncheck, stringer, modernize, goreleaser. Run via `go tool <name>`
  or make targets
