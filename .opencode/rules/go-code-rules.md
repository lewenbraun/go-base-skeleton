# Go Code Rules (skeleton)

Mandatory before editing any `.go` file. Style baseline is the
golang-code-style and golang-naming skills; this file adds skeleton rules.

## Hard gates

Every change must pass `make -C project check` (go vet + golangci-lint +
tests + build). Configured limits are law, not suggestions:
...
- errcheck: every error is either handled, wrapped with `%w`, or explicitly
  discarded as `_ =` (only for best-effort side effects)
...
- Errors: `fmt.Errorf("...: %w", err)` for wrapping, sentinel `errors.New`
  for static conditions. Compare with `errors.Is/As`. Error strings are
...
- stdlib `testing` only, table-driven with `t.Run` subtests and `t.Parallel`
  (testify is available as a skill but NOT a dependency of the skeleton)
- Pure logic gets tests. Process and network code is covered manually —
  do not mock os/exec or net/http here
- Test file name mirrors the source: `cmd.go` -> `cmd_test.go`

## Formatting

`make -C project fmt` (gofmt + gofumpt + goimports via golangci-lint) before
every commit. CI/`make check` fails on unformatted code.
